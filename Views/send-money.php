...
require_once __DIR__ . '/../Controllers/Post/Post.php';
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title>Send Money - PWA HTML Template</title>
    ...
    <form action="" method="post">
      <div class="px-6 flex justify-between items-center">
        ...
        <div class="flex-1 flex justify-center items-center pr-12">
          <p class="text-2xl font-semibold">Envoyer de L'argent</p>
        </div>
      </div>
      <div class="px-6 pt-8 flex flex-col gap-4">
        ...
        <select name="contact" id="" required class="p-4 bg-n20 border border-n40 dark:bg-darkN20 dark:border-darkN40 flex justify-between items-center rounded-xl">
          <?php 
          $query = $connection->prepare('SELECT * FROM utilisateur WHERE telephone != :telephone');
          $query->bindValue(':telephone', $phone, PDO::PARAM_STR);
          $query->execute();
          $contacts = $query->fetchAll(PDO::FETCH_ASSOC);
          foreach ($contacts as $contact) {
              $contactName = htmlspecialchars($contact['nom'], ENT_QUOTES, 'UTF-8');
              $contactPhone = htmlspecialchars($contact['telephone'], ENT_QUOTES, 'UTF-8');
              echo "<option value=\"{$contactPhone}\">{$contactName} ({$contactPhone})</option>";
          }
          ?>
        </select>
          ...
          <div class="relative">
            ...
            <div
              ...
              <ul class="flex flex-col gap-2">
                ...
                <li
                  class="text-xs text-n500 dark:text-darkN500 cursor-pointer item">
                  Wallet
                </li>
              </ul>
            </div>
        ...
        <div class="flex flex-col justify-start">
          ...
          <div
            class="p-4 bg-white border border-n40 dark:bg-darkN20 dark:border-darkN40 rounded-xl">
            <input
              name="amount"
              type="number"
              placeholder="|"
              value="1"
              min="1"
              max="<?php echo $userAccount ?>"
              class="w-full bg-transparent outline-none placeholder:text-black text-[52px] dark:placeholder:text-white dark:text-white font-bold" />
          </div>
        </div>
        <!-- <div class="flex justify-between items-center">
          <div class="flex justify-start items-center gap-2">
            <p class="text-n500 text-sm dark:text-darkN500 font-semibold">
              Bank Fee
            </p>
            <i class="ph ph-info text-g300 text-xl !leading-none"></i>
          </div>
          <p class="text-n500 text-sm dark:text-darkN500">$ 2.00</p>
        </div> -->
      </div>
      <div class="my-3 flex px-6 pt-8">
        <input type="submit" name="EnvoyerArgent" class="flex-1 py-3 bg-g300 text-white text-center rounded-xl" value="envoyer de l'argent" />
      </div>
    </form>
...
<!-- Mirrored from softivuspro.com/html/bankux/main-demo/send-money.html by HTTrack Website Copier/3.x [XR&CO'2014], Sun, 09 Nov 2025 18:42:51 GMT -->
